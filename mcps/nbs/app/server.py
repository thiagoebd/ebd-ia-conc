...
Autenticação via Bearer token (MCP_NBS_TOKEN).
...
MCP_HOST = os.environ.get("MCP_NBS_HOST", "0.0.0.0")
MCP_PORT = int(os.environ.get("MCP_NBS_PORT", "8990"))
MCP_TOKEN = os.environ.get("MCP_NBS_TOKEN", "")
MCP_LOG_DIR = Path(os.environ.get("MCP_NBS_LOG_DIR", str(PROJECT_ROOT / "logs" / "mcp-oracle")))
...
class BearerAuthMiddleware(BaseHTTPMiddleware):
    """Valida header Authorization: Bearer <MCP_NBS_TOKEN>."""
...
def main() -> int:
    if not MCP_TOKEN:
        log.error("startup_failed", reason="MCP_NBS_TOKEN não configurado no .env")
        print("ERRO: MCP_NBS_TOKEN não está definido no .env", file=sys.stderr)
        return 1
