...
DEALERNET_QUERY_TOOL = {
    ...
    "description": (
        "Executa uma query SQL READ-ONLY (T-SQL) contra o DealerNet Workflow "
        "(SQL Server) — 30 concessionarias: Toyota(Thai), Jeep(Way), Fiat(VM/Viale), "
        "Ford(Antares), Hyundai(Miso), Leapmotor. "
        "DIALETO T-SQL: use TOP n (nao FETCH FIRST), GETDATE() (nao SYSDATE), "
        "ISNULL (nao NVL), CONVERT/FORMAT (nao TO_DATE/TO_CHAR). "
        "SEMPRE filtre Empresa_Codigo. Para venda: NotaFiscal_Status=\'EMI\' AND "
        "NotaFiscal_Movimento=\'S\'. Tabelas: NotaFiscal, NotaFiscalItem, OS, "
        "OficinaServico, OficinaProduto, Veiculo, Pessoa, Produto, Titulo, Empresa. "
        "Base grande (ProdutoPreco tem 800M linhas) — sempre com WHERE restritivo. "
        "Retorna ate 1000 linhas."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
...
async def execute_dealernet_query(
    sql: str,
    max_rows: int = 100,
    user_identifier: str = "service@ebd.ia",
    canal: str = "test",  # MCP aceita: whatsapp|telegram|web|test
) -> dict[str, Any]:
    headers = {"Authorization": f"Bearer {settings.mcp_dn_token}"}
    try:
        async with streamablehttp_client(settings.mcp_dn_url, headers=headers) as (r, w, _):
...
def format_result_for_claude(payload: dict) -> str:
    ...
    if status != "ok":
        ...
        code = err.get("code", "?")
        msg = err.get("message", "?")
        _log.warning("dealernet_query FAIL code=%s msg=%s", code, str(msg)[:200])
    ...
    if not rows:
        _log.info("dealernet_query OK rows=0 elapsed=%.0fms", elapsed)
    ...
    _log.info("dealernet_query OK rows=%d elapsed=%.0fms truncated=%s", len(rows), elapsed, truncated)
...
if __name__ == "__main__":
    async def main():
        ...
        print(f"Testando MCP em {settings.mcp_dn_url}...")
        result = await execute_dealernet_query(sql)
...
# enquanto ela não termina. Telegram continua usando execute_dealernet_query
...
async def execute_dealernet_query_streaming(
    sql: str,
    max_rows: int = 100,
    user_identifier: str = "service@ebd.ia",
    canal: str = "web",
    ...
    """
    import time
    task = asyncio.create_task(execute_dealernet_query(
